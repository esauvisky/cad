...
difference()
{
    ...
    // reset button
    translate( [ 62.5 , 37.3 , 0 ] ) cylinder( h = 6.2, r = 3.5, $fn=30);
    ...
    // ORIGINAL PRUSA text
    translate([-57+5,51,0.6]) rotate([180,0,0]) linear_extrude(height = 2)
    { text("POKEMOD",font = "helvetica:style=Bold", size=7, center=true); }

    translate([-72,52,0.6]) rotate([180,0,0]) linear_extrude(height = 2)
    { resize([15,15,15]) import("./pokemod.svg"); }
    //translate([-18,51,0.6]) rotate([180,0,0]) linear_extrude(height = 2)
    //{ text("PRUSA",font = "helvetica:style=Bold", size=11, center=true); }
    translate( [ -56+6 , 40.5 , -0.4 ] )  cube( [ 45 , 1.6 , 1 ] );
    translate( [ -56+6 , 41.3 , -0.4 ] ) cylinder( h = 1, r = 0.8, $fn=30);
    translate( [ -11+6 , 41.3 , -0.4 ] ) cylinder( h = 1, r = 0.8, $fn=30);

    // front cleanup
    translate( [ -100 , -64.6 , 0 ] ) cube( [ 200 , 50 , 50 ] );

    // X sign on reset button
    translate( [ 56.5 , 39.6, -1 ] ) rotate([0,0,180]) mirror([1,0,0]) linear_extrude(height = 2)  text("IQ47",font = "helvetica:style=Bold", size=3);
